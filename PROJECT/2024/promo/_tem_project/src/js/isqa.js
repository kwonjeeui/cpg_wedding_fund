    ...
    const tooltip = {
        omniLoad: function () {
            ...
            $secWrap.find('[data-omni]').each(function(idx){
                ...
                const omniType = $this.attr('data-omni-type') === 'microsite' ? '' : 'microsite 없음, ';

                $target.append(`<li class="item item${idx} ${omni === '' || omniType === 'microsite 없음, ' ? 'null' : ''}"><input type="checkbox" ${omni === '' || omniType === 'microsite 없음, ' ? 'checked' : ''}><span class="pt_desc">${omniType}${omniData}</span></li>`);
