...
export const accordian = {
    ...
    toggle(params) {
        ...
        PT_STATE.$PROJECT.off('click.toggleAcc').on('click.toggleAcc', '[data-role-accordian]', function (e) {
            e.preventDefault();
            const $this = $(this);
            const { target, speed = data.opt.speed, openFocus = data.opt.openFocus, group, scroll = data.opt.scroll } = _.findItem(data.params, 'el', `[data-role-accordian="${$this.attr('data-role-accordian')}"]`);
            const $target = $(target);
            ...
            $('html, body').stop().animate({ scrollTop: $target.is(':visible') && openFocus ? $target.offset().top + _.pxToVw(scroll[0], scroll[1]) : $this.offset().top + _.pxToVw(scroll[0], scroll[1]) }, 500);
