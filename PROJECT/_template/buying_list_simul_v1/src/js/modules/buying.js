...
function setOption(buying, dataList, groupList){
    dataList.forEach(function(item){
        let arrColor=[],arrOptA=[],arrOptB=[],arrOptAShow=[],arrOptBShow=[],arrOptAHide=[],arrOptBHide=[];

        groupList[item.grp].forEach(function(groupItem){
            if(!!groupItem.optCdCo && groupItem.optCdCo != '-') {
                arrColor.push(groupItem.optCdCo+'|'+groupItem.optNmCo+'|'+groupItem.optCo);
            }
        ...
        let nextItem = [];
        let arrOptions = ['optCo', 'optCdA', 'optCdB'];
        ...
        if(arrOptAShow.length>0) item['arrOptAShow'] = uniqueArr(arrOptAShow);
        if(arrOptBShow.length>0) item['arrOptBShow'] = uniqueArr(arrOptBShow);
        if(arrOptAShow.length>0) item['arrOptAHide'] = diffArr(uniqueArr(arrOptA), uniqueArr(arrOptAShow));
        if(arrOptBShow.length>0) item['arrOptBHide'] = diffArr(uniqueArr(arrOptB), uniqueArr(arrOptBShow));
...
export class Buying {
    ...
    setData() {
        ...
        if(params.paging.use){
            buying.state.paging = JSON.parse(JSON.stringify(paging));
    ...
    getListPaging(listDefault){
        const buying = this;
        const params = buying.params;
        const paging = buying.state.paging;

        if(!listDefault){
            const list = params.pdList;
            const visibleData = list.filter((item) => item.hdn.trim().toUpperCase() != 'O');
            listDefault = !!visibleData[0].def ? visibleData.filter((item) => item.def.trim().toUpperCase() == 'O') : [];
        }
